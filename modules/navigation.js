...
export function bindAppUi() {
  ...
  el.printEstimate.addEventListener('click', () => {
    ...
    if (!saved) return;
    printEstimate({ ...saved, ...collectEstimateFromForm(), id: saved.id });
  ...
  el.printInvoice.addEventListener('click', () => {
    ...
    if (!saved) return;
    printInvoice({ ...saved, ...collectInvoiceFromForm(), id: saved.id, payments: saved.payments || [] });
  ...
  if (el.printContract) el.printContract.addEventListener('click', () => {
    ...
    if (!saved) return;
    printContract({ ...saved, ...collectContractFromForm(), id: saved.id });
